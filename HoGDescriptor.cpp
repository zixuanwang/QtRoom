...
void HoGDescriptor::compute(const cv::Mat& image, const cv::Rect& bbox, std::vector<float>& histogram){
    ...
    for(int row = 0; row < m_row_cell_count; ++row){
        for(int col = 0; col < m_column_cell_count; ++col){
            cv::Mat sub_image = image(cv::Rect(bbox.x + col * cell_width, bbox.y + row * cell_height, cell_width, cell_height));
            std::vector<float> sub_histogram;
            compute_hog(sub_image, sub_histogram);
...
void HoGDescriptor::compute_hog(const cv::Mat& image, std::vector<float>& histogram){
